  ...
  return (
    <div
      className={cn("space-y-3 relative z-[1]", {
        "before:absolute before:left-2 before:right-2 before:top-0 before:h-[calc(100%-23px)] before:w-[2px] before:bg-secondary before:rounded before:z-[-1]":
        ...
        >
          ...
          {hasChildren && (
            <button
              onClick={handleExpand}
              className={cn(
                "transform transition-all duration-200 flex-none leading-none menu-arrow rotate-180",
                {
                  "rotate-90 rotate-90": expandedKeys.includes(id),
                }
              )}
            >
              {endContent}
          ...
          {expandedKeys.includes(id) &&
            children.map((child) => (
              <React.Fragment key={child.id}>
                {showLine && (
                  <span className="absolute -left-3 right-3 mt-1.5">
                    <svg
